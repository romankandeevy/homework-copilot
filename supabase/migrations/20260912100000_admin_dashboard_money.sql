-- Дашборд с деньгами наверху.
--
-- Владелец заходит, чтобы увидеть, сколько заработал, а видел тревоги и
-- счётчики задач. Модель у сервиса - кошелёк с предоплатой, поэтому денег
-- три вида, и все три теперь есть в ответе:
--   пришло     - подтверждённые пополнения минус возвраты (revenue);
--   отработано - списано с кошельков за решения и чат (consumption);
--   должны     - что лежит на кошельках учеников и ещё не потрачено
--                (walletLiability): это обязательство, а не прибыль.
-- Плюс платящие ученики, первые оплаты и число пополнений - для среднего
-- чека. Везде без аккаунтов администраторов (private.is_staff).
