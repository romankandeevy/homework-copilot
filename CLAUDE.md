...
Ролики витрины собраны кодом: `src/promo/` - сцены как функция времени T,
без CSS-анимаций. Студия с таймлайном - `http://localhost:5173/?promo=1`
(только в разработке), `&format=tall` - вертикальный кадр для телефона.
Менять текст и тайминги - там, потом `npm run promo:render`.

Форматов два: 1920×1080 (`hero.mp4`, `promo.mp4`) и 1080×1350 для телефона
(`hero-tall.mp4`, `promo-tall.mp4`). Витрина выбирает файл по ширине экрана,
порог - 640 px, тот же, что у вёрстки первого экрана.
