...
- Отказ шлюза (`code: 524` и подобные) остаётся транзиентным и повторяется
  один раз — это тоже проверено тестом «выдаёт решение, когда одно
  семейство лежит целиком». Повтор укладывается в `retryDeadline`
  и на ожидание ученика больше не влияет.
