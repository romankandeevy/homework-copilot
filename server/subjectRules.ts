...
const formatBySubject: Record<string, readonly string[]> = {
  mathematics: [
    ...
    'Уравнение - столбиком, по строке на преобразование; если в классе принято, последней строкой проверка подстановкой. Ответ - полным ответом на вопрос: «68 банок варенья».',
  ],
  algebra: [
    'Уравнение, неравенство, система - столбиком: исходная запись, под ней каждое преобразование своей строкой, последняя строка - найденное значение или множество.',
    'Квадратное уравнение: дискриминант отдельной строкой «D = b² - 4ac = …», затем корни «x₁ = …», «x₂ = …». Посторонние корни отбрасываются строкой с причиной.',
  ...
  geometry: [
    ...
    'Символы - ∠, △, ∥, ⟂, °. На чертёж ссылаются буквами точек, числа из условия стоят на чертеже.',
  ],
  physics: [
    'Запись «Дано - СИ - Найти - Решение - Ответ». В «Дано» каждая величина с единицей, перевод в СИ в той же строке: «m = 200 г = 0,2 кг», «ν = 0,5 Гц». В «Найти» - искомая величина: «t - ?».',
  ...
  biology: [
    ...
    'Вопрос параграфа - связный ответ терминами, два-пять предложений.',
  ],
  informatics: [
    ...
    'Перевод между системами счисления - столбиком, у каждого числа основание подстрочной цифрой: «101101₂ = 45₁₀».',
  ],
  russian: [
  ...
  literature: [
    'Ответ на вопрос - связный текст: первое предложение прямо отвечает на вопрос его же словами, дальше - доводы с опорой на эпизоды и цитаты, последний абзац - вывод «Таким образом, …».',
    'На вопрос параграфа - два-пять предложений. Сочинение - объём, который задан; если нужен план, он идёт перед текстом строкой «План:» и пунктами.',
  ],
  english: [
    'Перевод и ответы на вопросы - парами «English - русский» через дефис: «What have you got in your school bag? - Что у тебя в портфеле?». Диалог - построчно, реплика и её перевод.',
  ...
  history: [
    'Ответ на вопрос параграфа - связный текст: первое предложение прямо отвечает на вопрос, дальше - даты, участники, причины и следствия, последнее - вывод «Таким образом, …». Два-пять предложений на вопрос.',
  ],
  social: [
    'Ответ на вопрос - связный текст: первое предложение прямо отвечает на вопрос, дальше - термин с определением и пример из жизни, последнее - вывод. Два-пять предложений на вопрос.',
