...
function qualityIssues(diagram: HomeworkDiagram, condition: string, steps: string[]) {
  const solution: HomeworkSolution = {
    ...
    /* Разбор темы, а не пересказ задачи: с 6 сентября это требование
       проверяется кодом, и три мысли - его минимум. */
    explanation: [
      'В построении держатся за опорную фигуру: сначала она, потом всё, что на неё опирается.',
      'Признак такой задачи: спрашивают не величину, а расположение точек и линий.',
      'Частая ошибка - строить по порядку слов условия, а не от опорной фигуры.',
    ],
    steps,
    answer: '',
    diagram,
