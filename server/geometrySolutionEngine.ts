...
const authorInstructions = [
  ...
  'Каждое преобразование - отдельная строка steps, одна под другой, как в столбике тетради. Нельзя склеивать ход решения стрелками в ленту: «2x > -5 ⇒ x > -2,5 ⇒ x ∈ (-2,5; +∞)» - это три строки, а не одна. Больше одного ⇒ или ⇔ в строке не ставь.',
...
export function validateSolutionQuality(solution: HomeworkSolution) {
  ...
  /* Цепочка преобразований в одну строку.
     ...
     Один ⇒ в строке - это вывод из посылки, а не лента. */
  const chained = steps.find((line) => (line.match(/[⇒⇔→]/gu)?.length ?? 0) > 1)
  if (chained) {
    issues.push('Преобразования склеены в одну строку через ⇒: каждое пиши отдельной строкой, как в тетради')
  }
