...
const authorInstructions = [
  ...
  // Итог и так приходит отдельным полем answer и показывается отдельным
  // блоком на странице. Модель дублировала его последней строкой steps —
  // на странице это читалось как тройной повтор одного числа.
  'Не пиши в steps отдельную строку «Ответ: …» — итог уже передаётся полем answer. Последний шаг steps — последнее действие или вычисленное значение, без слова «Ответ» в начале строки.',
