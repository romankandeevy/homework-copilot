...
//
// Верно решают почти все - расходятся цена и время. Отсюда порядок:
// gemini-3-6-flash-openai первой (дешевле всех и быстрее большинства),
// gpt-5-6-terra второй - это единственная не-Gemini модель, которая
// одновременно отвечает верно и стоит разумно, и она страхует от отказа
// целого семейства. Дальше остальные Gemini.
//
...
// шлюз не доносит для Claude строгую схему (запрос с tools отвечает 503),
// а без схемы формат решения держать нечем; grok-4-6 - на трудной задаче
// шлюз отдал HTML вместо ответа.
//
...
// не держится).
export const defaultHomeworkModels = ['gemini-3-6-flash-openai', 'gpt-5-6-terra', 'gemini-3-5-flash-openai', 'gemini-3-pro'] as const
