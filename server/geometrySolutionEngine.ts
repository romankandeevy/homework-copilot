...
//   gemini-3-pro              83 с   строгую схему решателя не принял
//   gpt-5-6-luna             107 с   6864 и 143/2835 — неверно
//
// Отсюда порядок: первым идёт тот, кто отвечает верно, дальше — быстрый
// запасной. gemini-3-pro третьим: на обычный запрос он отвечает за 7 секунд,
// а на строгую схему решателя — нет, и вернётся вместе с её поддержкой.
//
// gpt-5-6-luna из пула убран совсем: единственная модель не-Gemini у шлюза,
// и она же единственная, кто дал неверный ответ. Держать её запасной значит
// обещать ученику решение и отдать ошибку - лучше честный отказ.
//
