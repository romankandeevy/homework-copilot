...
const taskThreeDraft = {
  ...
  explanation: [
    'Три прямые на плоскости пересекаются либо попарно, либо все в одной точке — других расположений нет.',
    'Признак такой задачи: спрашивают «сколько точек», а расположение прямых не задано.',
    'Частая ошибка — назвать один ответ: случаев здесь два, и оба нужно разобрать.',
  ],
  steps: [
    'a ∩ b = A; b ∩ c = B; a ∩ c = C, значит n = 3.',
    'a ∩ b ∩ c = O, значит n = 1.',
  ],
...
const photoDraft = {
  ...
  goal: { title: 'Найти', text: '∠C' },
  explanation: [
    'Сумма углов треугольника равна 180° — это следует из параллельной прямой через вершину.',
    'Признак такой задачи: известны два угла треугольника, спрашивают третий.',
    'Частая ошибка — сложить известные углы и записать сумму в ответ вместо разности.',
  ],
...
const russianDraft = {
  ...
  explanation: [
    ...
    'Признак задания: просят разобрать слово и назвать способ образования.',
    'Частая ошибка — принять приставку за часть корня, поэтому сначала находят корень.',
  ],
...
describe('homework solver', () => {
  ...
  it('publishes safe configuration status and CORS only for approved origins', async () => {
    ...
    expect(http.response.statusCode).toBe(200)
    expect(http.body()).toEqual({ provider: 'kie.ai', model: 'gemini-3-5-flash-openai', configured: true, engineVersion: homeworkSolutionEngineVersion })
