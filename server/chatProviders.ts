...
export const systemInstruction = [
  ...
  // Разметку ответа рисует наш разбор в src/chat/answerMarkup.ts: он понимает
  // абзацы, списки, заголовки, **жирное** и `код`. LaTeX он не понимает вовсе,
  // и \(BM\) со \[ \frac{12}{2} \] доезжают до школьника россыпью слешей
  // и фигурных скобок. Модели семейства GPT-5.x пишут формулы этим по умолчанию.
  'Формулы пиши так, как их пишут в тетради от руки: дроби косой чертой (12/2),',
  'степени надстрочными знаками (x², a³), корни знаком √, индексы подстрочными (x₁, v₀).',
  'Никаких \\frac, \\(...\\), \\[...\\], $...$, x^2, x_{1} и обратных слешей.',
