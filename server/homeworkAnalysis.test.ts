...
const draft = {
  ...
  explanation: [
    ...
    'Признак задания: просят подчеркнуть члены предложения, а не разобрать слово по составу.',
    'Частая ошибка — принять дополнение за подлежащее, поэтому вопрос задают от сказуемого.',
  ],
