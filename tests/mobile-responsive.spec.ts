...
test.describe('адаптация под телефон', () => {
  ...
  test('регистрация помещается на экран и прокручивается внутри диалога', async ({ page }) => {
    ...
    // Кнопка не гаснет (аудит 16 сентября, Г1): нажатие называет первое
    // незаполненное поле и ставит в него курсор.
    const createAccount = page.getByRole('button', { name: 'Создать аккаунт' })
    await expect(createAccount).toBeEnabled()
    await createAccount.click()
    await expect(page.locator('.account-form-message.is-error')).toHaveText('Введи имя')
    ...
    await createAccount.click()
    await expect(page.locator('.account-form-message.is-error')).toContainText('В пароле не хватает')
