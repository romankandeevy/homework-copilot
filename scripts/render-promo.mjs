/* Рендер ролика первого экрана в mp4.

   Кадры считает React-сцена (src/promo/PromoFilm.tsx) от авторского времени T.
   Здесь Playwright открывает студию в режиме рендера, выставляет T кадр за
   кадром через window.promoControl.setTime и снимает экран, а ffmpeg собирает
   снимки в видео. Итог - public/hero.mp4 и постер public/hero-poster.jpg.

     node scripts/render-promo.mjs [--fps 60] [--scale 1] [--crf 14] [--poster-at 31]
...
const posterAt = Number(option('poster-at', 31))
const output = resolve(option('out', 'public/hero.mp4'))
const poster = resolve(option('poster', 'public/hero-poster.jpg'))
const port = 4179
...
try {
  ...
  await page.goto(`http://127.0.0.1:${port}/?promo=1&render=1`, { waitUntil: 'load' })
  ...
  console.log(`[promo] ${total.toFixed(1)} с, ${frames} кадров, ${fps} к/с, масштаб ${scale}, crf ${crf}`)
  ...
  for (let index = 0; index < frames; index += 1) {
    ...
    if (index % fps === 0) {
      ...
      process.stdout.write(`\r[promo] ${time.toFixed(0).padStart(3)} с из ${total.toFixed(0)} · ${elapsed} с прошло   `)
    }
  }
  ...
  console.log(`[promo] ${output} - ${(videoSize.size / 1024 / 1024).toFixed(1)} МБ`)
  console.log(`[promo] ${poster} - ${(posterSize.size / 1024).toFixed(0)} КБ (кадр на ${posterAt} с)`)
} finally {
