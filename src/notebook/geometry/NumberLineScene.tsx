...
/* Координатная прямая.
   ...
   картинка: ось со стрелкой, выколотая или закрашенная точка на границе,
   жирный луч по множеству решений. Пунктов в задании обычно четыре, поэтому
   прямых на чертеже столько же и стоят они друг под другом, как в тетради.
...
function NumberLineRow({ line, y }: { line: Line; y: number }) {
  ...
  /* Жирный луч не заходит на остриё стрелки: иначе конец оси распухает и
     стрелка читается не как направление, а как утолщение самого луча. */
  ...
  return (
    <g className="number-line-row">
      ...
      {keyed(line.regions, (region) => `region-${region.from}-${region.to}`).map(({ key, item: region }) => {
        ...
        return <path className="number-line-region" d={`M ${from} ${y} L ${to} ${y}`} key={key} />
      })}
