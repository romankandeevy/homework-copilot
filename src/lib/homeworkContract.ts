...
/* Координатная прямая.
   ...
   «Изобразите на координатной прямой множество решений» - отдельный
   школьный чертёж: горизонтальная ось со стрелкой, выколотая или закрашенная
   точка на границе и жирный луч по множеству решений. Ни поле 0..100, ни
   координатная плоскость его не дают: 9 сентября неравенства а-г с прода
   ушли ученику без чертежа вовсе - модель решила, что промежутков в ответе
   достаточно.

   Прямых столько, сколько пунктов в задании: у каждой свой заголовок «а)».
   Числа - настоящие значения, подпись value отдельно, потому что 2/7 в
   тетради пишут дробью, а не как 0,2857. */
export type HomeworkNumberLine = {
  lines: Array<{
    /** Заголовок прямой: «а)», «б)». Пусто, если прямая одна. */
    label: string
    ...
    marks: Array<{
      value: number
      /** Подпись под точкой в тетрадной записи: «-2,5», «2/7». */
      label: string
      /** true - точка входит в множество (≤, ≥), рисуется закрашенной. */
      filled: boolean
    }>
    /** Закрашенные промежутки; null - уход в бесконечность. */
...
/* Сколько шло решение и сколько за него списано - ученику под решением.
   Живёт рядом с решением, а не в нём: решение лежит в общем каталоге, и
   чужая сумма не должна попадать тому, кто откроет ту же запись. */
export type SolveReceipt = {
  seconds: number
  /** 0 - бесплатное решение гостя или уже купленная запись. */
  kopecks: number
  /** Запись уже была у ученика: модель не звали, денег не брали. */
  reused?: boolean
}
...
export type HomeworkDiagram = {
  ...
  /** Для kind = number-line: множества решений на координатной прямой. */
  numberLine?: HomeworkNumberLine
}
...
export type HomeworkSolution = {
  ...
  explanation?: string[]
  steps: string[]
  /* Программа целиком, если задание просило её написать.

     7 сентября на проде информатика вернула код на Python, втиснутый в две
     строки тетради: «Python: count = {0:1}; s = 0; ans = 0». В строке
     тетради нет ни отступов, ни переносов, ни моноширинного шрифта - класть
     туда программу было некуда, и это дыра в контракте, а не промах модели.

     Необязательное: решения, сохранённые до появления поля, остаются
     читаемыми, как и объяснение. */
  code?: {
    language: string
    text: string
  }
