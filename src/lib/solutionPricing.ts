/* Цена решения. Все суммы — целые копейки, как и баланс в базе.
   ...
   провайдера. Тогда один разбор стоил 3–4 вызова модели: два независимых
   прохода, при расхождении рецензент, иногда починка чертежа.

   4 сентября проход стал один, зато более дорогой моделью, а проверку
   вместо рецензента делает код. Цену это не подняло: вызовов на задачу
   меньше втрое, и запас, заложенный ниже, покрывает разницу в тарифе.
   Замер себестоимости повторить после первой недели на новом пуле.
...
/* Во что нам обходится кредит шлюза. Кредит KIE стоит $0.005; при курсе
   ...
   Нужен, чтобы расход на решение писался в журнал сразу в копейках:
   кредиты сравнивать с ценой нельзя, а копейки - можно. */
