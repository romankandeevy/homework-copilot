...
/* Стадии — то, что решатель действительно сообщает о себе, а не бегунок,
   ...
   зарезервирована; `solving` — когда запущен проход модели; `checking` —
   когда решение проверяется правилами предмета; `writing` — когда решение
   получено и сохраняется. */
export const solutionStages = [
  { id: 'reading', title: 'Читаем', note: 'Разбираем, что дано и что найти' },
  { id: 'solving', title: 'Решаем', note: 'Разбираем задачу и пишем ход решения' },
  { id: 'checking', title: 'Проверяем', note: 'Сверяем запись с правилами предмета' },
